...
public class FirstPersonController : MonoBehaviour
{
    ...
    #region interactions
    public float playerActivationDistance = 1.0f;
    ...
    public float playerHitRange = 10f;
    ...
    private void Awake()
    {
        ...
        shng = GetComponentInChildren<AudioSource>();
    ...
    private void Update()
    {
        ...
        #region interactions
        RaycastHit hit;
        active = Physics.Raycast(playerCamera.transform.position, playerCamera.transform.TransformDirection(Vector3.forward), out hit, playerActivationDistance);
        ...
        if(active == true)
        {
            ...
            {
                if (HasParameter("interact", hit.transform.gameObject.GetComponent<Animator>()))
                {
                    Debug.Log("interactable");
                }
                GameObject[] enemies;
                ...
                {
                    hit.transform.gameObject.GetComponent<Animator>().SetTrigger("interact");
                    foreach(GameObject enemy in enemies)
                    {
                        Debug.Log("enemy Found");
                        enemy.SendMessage("gateOpened");
                    }
                }
            } 
            
            if(hit.transform.gameObject.tag == "FinalButton")
            {
                if(Input.GetKeyDown(interactkey))
                {
                    StartCoroutine(Win());
    ...
    IEnumerator Win()
    {
        ...
        if (living)
        {
            ...
            playerCanMove = false;
            cameraCanMove = false;
            shng.PlayOneShot(victorysound);
            yield return new WaitWhile(() => shng.isPlaying);
    ...
    IEnumerator die()
    {
        if(living)
        {
            ...
            playerCanMove = false;
            cameraCanMove = false;
            playerCamera.BroadcastMessage("fadeInToBlack");
            shng.PlayOneShot(deathsound);
            yield return new WaitWhile(() => shng.isPlaying);
            SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
        }
    }
    ...
    public void HitByEnemy(float incomingDamage)
    {
    ...
    private void attack()
    {
        ...
        shng.PlayOneShot(attacksound);
        RaycastHit hit;
        bool active = Physics.Raycast(rb.transform.position, rb.transform.forward, out hit, playerHitRange);
        Debug.DrawRay(rb.transform.position, rb.transform.forward * playerHitRange, Color.green, 2);
        if (active == true)
        {
            if (hit.collider.tag == "enemy")
            {
                hit.transform.SendMessage("HitByPlayer", 5f);
            }
        }
    }    
    ...
    private void Crouch()
    {
        ...
        else if(isSprinting && !powerSlide)
        {
            transform.localScale = new Vector3(originalScale.x, crouchHeight, originalScale.z);            transform.localScale = new Vector3(originalScale.x, crouchHeight, originalScale.z);
            Vector3 zoom = rb.transform.forward * sprintSpeed * slideMultiplier;
            rb.AddForce(zoom, ForceMode.VelocityChange);
            walkSpeed *= speedReduction;
            isCrouched = true;
            isSprinting = false;
            powerSlide = true;
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        #region Interaction Balance

        GUILayout.Label("Interaction Balance", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));

        fpc.attackKey = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("attack Key", "Determines what key is used to attack."), fpc.attackKey);
        ...
        fpc.playerActivationDistance = EditorGUILayout.FloatField(new GUIContent("Interaction Distance", "Determines how fast the player will move while walking."), fpc.playerActivationDistance);
        fpc.health = EditorGUILayout.FloatField(new GUIContent("health", "Determines how much health the player has."), fpc.health);
        fpc.damage = EditorGUILayout.FloatField(new GUIContent("damage", "Determines how much damage the player does."), fpc.damage);
        fpc.playerHitRange = EditorGUILayout.FloatField(new GUIContent("playerHitRange", "Determines how far the player can hit you from."), fpc.playerHitRange);

        EditorGUILayout.PrefixLabel(new GUIContent("Attack Sound", "Sound of attack"));
        fpc.attacksound = (AudioClip)EditorGUILayout.ObjectField(fpc.attacksound, typeof(AudioClip), false);
        EditorGUILayout.PrefixLabel(new GUIContent("Victory Sound", "Sound of victory"));
        fpc.victorysound = (AudioClip)EditorGUILayout.ObjectField(fpc.victorysound, typeof(AudioClip), false);
        EditorGUILayout.PrefixLabel(new GUIContent("Death Sound", "Sound of death"));
        fpc.deathsound = (AudioClip)EditorGUILayout.ObjectField(fpc.deathsound, typeof(AudioClip), false);
        ...
        fpc.walkSpeed = EditorGUILayout.FloatField(new GUIContent("Walk Speed", "Determines how fast the player will move while walking."), fpc.walkSpeed);
        fpc.slideMultiplier = EditorGUILayout.FloatField(new GUIContent("slideMultiplier", "Determines how much faster you slide."), fpc.slideMultiplier);
        GUI.enabled = true;
        ...
        fpc.sprintSpeed = EditorGUILayout.FloatField(new GUIContent("Sprint Speed", "Determines how fast the player will move while sprinting."), fpc.sprintSpeed);
        ...
        fpc.jumpPower = EditorGUILayout.Slider(new GUIContent("Jump Power", "Determines how high the player will jump."), fpc.jumpPower, 0.1f, 100f);
        fpc.gravity = EditorGUILayout.Slider(new GUIContent("Gravity", "Determines how fast the player will fall."), fpc.gravity, 0.1f, 100f);
        GUI.enabled = true;
